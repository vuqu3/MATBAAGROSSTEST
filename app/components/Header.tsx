...
export default function Header() {
  ...
  return (
    <header className="bg-white shadow-md sticky top-0 z-50 overflow-visible">
      ...
      {isCartOpen && session?.user?.role !== 'SELLER' && (
        <div className="fixed right-0 top-0 h-screen z-[100] w-[350px] max-w-[350px] bg-white shadow-2xl flex flex-col">
            <div className="flex items-center justify-between px-3 py-2.5 border-b border-gray-200">
              <div className="flex items-center gap-2">
                <ShoppingCart size={18} className="text-[#FF6000]" />
                <span className="font-extrabold text-[#484848]">Sepet Özeti</span>
                {cartCount > 0 ? (
              ...
              <button
                type="button"
                onClick={closeCart}
                className="inline-flex items-center justify-center min-h-9 min-w-9 rounded-lg hover:bg-gray-50 text-[#484848]"
                aria-label="Kapat"
              >
                <X size={20} />
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-3">
              {cartItems.length === 0 ? (
                <div className="text-center text-sm text-gray-500 py-10">Sepetiniz boş.</div>
              ) : (
                <div className="space-y-3">
                  {cartItems.map((it) => (
                    <div key={it.lineId} className="flex items-center gap-3 border border-gray-200 rounded-xl p-3">
                      <Link
                        href={`/urun/${encodeURIComponent(it.productId)}`}
                        className="w-12 h-12 rounded-lg overflow-hidden bg-gray-50 border border-gray-100 flex-shrink-0"
                        onClick={closeCart}
                        aria-label={it.name}
                      >
                        {it.imageUrl ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-xs text-gray-400">Ürün</div>
                        )}
                      </Link>
                      <div className="flex-1 min-w-0">
                        <Link
                          href={`/urun/${encodeURIComponent(it.productId)}`}
                          onClick={closeCart}
                          className="text-xs font-semibold text-gray-900 line-clamp-2 hover:text-[#FF6000] transition-colors"
                        >
                          {it.name}
                        </Link>
                        <div className="mt-2 flex items-center gap-1.5">
                          <button
                            type="button"
                            onClick={() => {
                              const currentQuantity = Number(it.quantity);
                              const newQuantity = Math.max(1, currentQuantity - 1);
                              updateQuantity(it.lineId, newQuantity);
                            }}
                          ...
                          <div className="w-8 text-center text-xs font-semibold text-gray-700">{it.quantity}</div>
                          <button
                            type="button"
                            onClick={() => {
                              const currentQuantity = Number(it.quantity);
                          ...
                          <button
                            ...
                            className="ml-2 w-6 h-6 rounded border border-gray-300 flex items-center justify-center hover:bg-red-50 text-red-600"
                            aria-label="Sil"
                          >
                            <Trash2 size={14} />
                          </button>
                      ...
                      <div className="text-right">
                        <p className="text-xs font-extrabold text-[#FF6000] whitespace-nowrap">
                          {it.totalPrice.toLocaleString('tr-TR')} TL
                        </p>
                      </div>
                    </div>
                  ))}
            ...
            <div className="border-t border-gray-200 p-3 space-y-2.5">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-bold text-gray-900">{totalAmount.toLocaleString('tr-TR')} TL</span>
              </div>
              <div className="flex items-center justify-between text-sm">
                <span className="text-gray-600">Kargo</span>
                <span className="font-bold text-gray-900">
                  {hasFreeShipping ? 'Ücretsiz' : `${shippingCost.toLocaleString('tr-TR')} TL`}
                </span>
              </div>
                  ...
                  <div className="flex items-center gap-2 text-amber-800">
                    ...
                    <div className="flex-1">
                      <p className="text-sm font-medium">
                        Sepetinize{' '}
                        <span className="font-bold">{remainingForFreeShipping.toLocaleString('tr-TR')} TL</span>{' '}
                        daha ekleyin, kargo bedava olsun!
                      </p>
                      <div className="mt-2 bg-amber-200 rounded-full h-2">
                        <div
                          className="bg-amber-600 h-2 rounded-full transition-all duration-300"
                          style={{
                            width: `${Math.min(100, (totalAmount / Math.max(1, freeShippingThreshold)) * 100)}%`,
                          }}
                        />
                      </div>
              ...
              <div className="flex items-center justify-between">
                <span className="font-extrabold text-gray-900">Toplam</span>
                <span className="font-extrabold text-[#FF6000] text-base">{grandTotal.toLocaleString('tr-TR')} TL</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Link
                  href="/sepetim"
                  onClick={closeCart}
                  className="inline-flex items-center justify-center rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm font-bold text-gray-800 hover:bg-gray-50"
                >
                  Sepete Git
                </Link>
                <Link
                  href="/sepetim/onay"
                  onClick={closeCart}
                  className="inline-flex items-center justify-center rounded-xl bg-[#FF6000] px-4 py-3 text-sm font-extrabold text-white hover:bg-[#e55a00]"
                >
                  Siparişi Tamamla
                </Link>
              </div>
