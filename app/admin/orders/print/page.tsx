...
export default function AdminOrderPrintPage() {
  return (
    ...
    >
      <AdminOrderPrintPageInner />
    </Suspense>
  );
}

function AdminOrderPrintPageInner() {
      ...
      <div className="print-overlay fixed inset-0 z-[9999] flex items-center justify-center bg-white">
        <div className="print-label w-[105mm] min-h-[148mm] max-w-[105mm] bg-white text-black p-5 flex flex-col gap-4 shadow-lg print:shadow-none">
          ...
          <div className="text-center flex-1">
            ...
            <p className="text-xl font-bold font-mono text-gray-900">#{order.barcode ?? order.id.slice(0, 8)}</p>
            {order.barcode && (
              <div className="flex justify-center mt-3">
                <Barcode value={order.barcode} height={40} width={1.3} fontSize={11} margin={0} />
              </div>
            )}
          </div>

          <div className="border border-gray-400 rounded p-3 text-sm">
            <p className="font-semibold text-gray-900 mb-1.5">Alıcı: {buyerName}</p>
            ...
            {buyerPhone && <p className="text-gray-800 mt-1 font-medium">Tel: {buyerPhone}</p>}
          </div>

          <div className="border-t border-gray-400 pt-3">
            <p className="text-xs font-semibold text-gray-700 uppercase tracking-wide mb-2">Paket İçeriği</p>
